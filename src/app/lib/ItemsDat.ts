...
class ItemsDat {
  ...
  private getWriteSize(items: ItemDefinition[]) {
    ...
    for (const item of items) {
      ...
      for (const key of keys) {
        const strBuf = item[key] as unknown as ExtendString;
        if (this.stringFields.includes(key) && strBuf.raw) {
          // calc length typeof item[key] === "string"
          size += strBuf.raw.length + 2;
        }
        if (typeof item[key] === "object" && !strBuf.raw) {
  ...
  public async encode() {
    ...
    for (const item of this.meta.items) {
      ...
      if (this.meta.version! >= 11) {
        await this.writeString(item.punchOptions || { raw: Buffer.alloc(0), value: "" }, item.id!);
        ...
        if (this.meta.version! >= 15) {
          ...
          await this.writeString(
            item.extraTexture || { raw: Buffer.alloc(0), value: "" },
            item.id!
          );
        }
        ...
        if (this.meta.version! >= 16) {
          await this.writeString(
            item.itemRenderer || { raw: Buffer.alloc(0), value: "" },
            item.id!
          );
        }
  ...
  ): Promise<ExtendString> {
    const len = this.data.readInt16LE(this.mempos);
    this.mempos += 2;

    if (!opts.encoded)
      return {
        raw: this.data.subarray(this.mempos, this.mempos + len) as Buffer,
        value: this.data.toString("utf-8", this.mempos, (this.mempos += len)) as string
      };
    else {
      const chars = [];
      for (let i = 0; i < len; i++) {
  ...
  private writeString(str: ExtendString, id: number, encoded: boolean = false): Promise<undefined> {
    return new Promise((resolve) => {
      ...
      this.data.writeUInt16LE(str.raw.length, this.mempos);
      this.mempos += 2;

      if (!encoded) {
        if (str.raw.length) this.data.fill(str.raw, this.mempos);

        this.mempos += str.raw.length;
      } else {
        const chars = [];

        for (let i = 0; i < str.value.length; i++)
          chars.push(str.value.charCodeAt(i) ^ this.key.charCodeAt((i + id) % this.key.length));
